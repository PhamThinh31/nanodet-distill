...
def color_aug_and_normDA(meta, kwargs):
    ...
    img_target = meta["img_target"].astype(np.float32) / 255
    

    if "brightness" in kwargs and random.randint(0, 1):
        img_target = random_brightness(img_target, kwargs["brightness"])

    if "contrast" in kwargs and random.randint(0, 1):
        img_target = random_contrast(img_target, *kwargs["contrast"])

    if "saturation" in kwargs and random.randint(0, 1):
        img_target = random_saturation(img_target, *kwargs["saturation"])
    # cv2.imshow('trans', img)
    # cv2.waitKey(0)
    img_target = _normalize(img_target, *kwargs["normalize"])
    meta["img_target"] = img_target
