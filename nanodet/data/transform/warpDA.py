...
class ShapeTransform:
    ...
    def __call__(self, meta_data, dst_shape):
        ...
        raw_img_target = meta_data["img_target"]
        height_target = raw_img_target.shape[0]  # shape(h,w,c)
        width_target = raw_img_target.shape[1]

        # center
        C = np.eye(3)
        C[0, 2] = -width_target / 2
        C[1, 2] = -height_target / 2
        ...
        if self.keep_ratio:
            dst_shape = get_minimum_dst_shape(
                (width_target, height_target), dst_shape, self.divisible
            )

        ResizeM = get_resize_matrix((width_target, height_target), dst_shape, self.keep_ratio)
        M_target = ResizeM @ M
        img_target = cv2.warpPerspective(raw_img_target, M, dsize=tuple(dst_shape))
        
        meta_data["img_target"] = img_target
        meta_data["warp_matrix_target"] = M_target
