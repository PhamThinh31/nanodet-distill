...
# from nanodet.util import Lion
from lion_pytorch import Lion

class TrainingTaskDA(LightningModule):
    """
    ...
    def __init__(self, cfg, evaluator=None, neadby_test=False):
        super(TrainingTaskDA, self).__init__()
        self.cfg = cfg
    ...
    def _preprocess_batch_input(self, batch,train=True):
        batch_imgs = batch["img"]
        if train:
            batch_imgs_tartget = batch["img_target"]
        ...
        if isinstance(batch_imgs, list):
            ...
            if train and isinstance(batch_imgs_tartget, list):
                batch_imgs_target = [img.to(self.device) for img in batch_imgs_tartget]
                batch_img_target_tensor = stack_batch_img(batch_imgs_target, divisible=32)
                batch["img_target"] = batch_img_target_tensor
        return batch
    ...
    def predict(self, batch, batch_idx=None, dataloader_idx=None):
        batch = self._preprocess_batch_input(batch,train=False)
        # with torch.no_grad():
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.weight_averager is not None:
            preds, _, loss, loss_states, _ = self.avg_model.forward_train(batch,is_train=False)
        else:
            preds, _, loss, loss_states, _ = self.model.forward_train(batch,is_train=False)
    ...
    def configure_optimizers(self):
        ...
        if name == 'Lion':
            optimizer = Lion(params=self.parameters(), **optimizer_cfg)
        else:
